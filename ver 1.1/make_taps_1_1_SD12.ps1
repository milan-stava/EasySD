...
# ----------------------------------------------------------------------
# Simple TAP containing EasySD_1_1_INSTALL.bin
#
# BASIC:
...
function Make-FullElementTap {
    ...
    $EasyPath  = Join-Path $Root "EasySD_1_1_EL.bin"
    $OutPath   = Join-Path $Root "EasySD_1_1_EL.tap"
    ...
    if ($easy.Length -gt 0x4000) {
        throw "EasySD_1_1_EL.bin is too large for 16K staging page: $($easy.Length) bytes"
    }
    ...
    # eLeMeNt EasySD loader must stay on the proven 101-byte SD variant.
    # If this changes, the bootstrap initialization was modified.
    if ($loader.Length -ne 101) {
        throw "EL128DIR: expected proven EasySD loader length 101 bytes, got $($loader.Length)"
    }
    ...
    Write-Host "EasySD_1_1_EL.tap : OK"
    Write-Host "  EasySD_1_1_EL.bin: $($easy.Length) bytes"
...
function Make-FullSlimTap {
    $BSROMPath = Join-Path $Root "BSROM140_EL.rom"
    $BSDOSPath = Join-Path $Root "BSDOS.rom"
    $EasyPath  = Join-Path $Root "EasySD_1_1_SLIM.bin"
    $OutPath   = Join-Path $Root "EasySD_1_1_SLIM.tap"
    ...
    if ($easy.Length -gt 0x4000) {
        throw "EasySD_1_1_SLIM.bin is too large for 16K staging page: $($easy.Length) bytes"
    }
    ...
    Write-Host "EasySD_1_1_SLIM.tap : OK"
    Write-Host "  EasySD_1_1_SLIM.bin: $($easy.Length) bytes"
    Write-Host "  Loader: $($loader.Length) bytes"
    Write-Host "  TAP size: $($tap.Length) bytes, blocks: $blocks"
    Write-Host ("  PAGE0=#{0:X4} PAGE1=#{1:X4} PAGE3=#{2:X4} START=#{3:X4}" -f $PAGE0,$PAGE1,$PAGE3,$START)
}
...
try {
    Make-SimpleEasyTap "EasySD_1_1_INSTALL.bin" "EasySD_1_1_INSTALL.tap" "EASYSD11"
    Make-FullElementTap
    Make-FullSlimTap
}
catch {
